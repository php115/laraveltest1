...
class EmailSender implements NotificationSenderInterface
{
    public function send(Notification $notification): void
    {
        ...
        if (app()->environment('production', 'local') && rand(1, 10) === 1) {
            throw new Exception("Email gateway timeout for notification #{$notification->id}");
        }

        Log::info("Email successfully sent to user {$notification->user_id}: {$notification->text}");
    }
}
